...
unset_lock() {
    ...
    [ -f "${INSTANCE_LIST}" ] && ! grep -q 'lzvpns_' "${INSTANCE_LIST}" 2> /dev/null && rm -f "${INSTANCE_LIST}" > /dev/null 2>&1
...
consistency_update() {
    ...
    if [ "${retval}" = "1" ]; then
        {
            echo "$(lzdate)" [$$]: Missing data item "${1}" in VPN "${4}" script file.
...
set_wan_access_port() {
    ...
    if ip rule list | grep "^${IP_RULE_PRIO_HOST}:" | grep -v all | grep -q "${router_local_ip}" \
        && ip rule list | grep "^${IP_RULE_PRIO_HOST}:" | grep all | grep -q "${router_local_ip}"; then
...
start_daemon() {
    ...
    if ps | grep "${VPN_DAEMON_SCRIPTS}" | grep -qv grep; then
        echo "$(lzdate)" [$$]: LZ VPNS daemon has been started. | tee -ai "${SYSLOG}" 2> /dev/null
    elif cru l | grep -q "#${START_DAEMON_TIMEER_ID}#"; then
        echo "$(lzdate)" [$$]: LZ VPNS daemon is starting... | tee -ai "${SYSLOG}" 2> /dev/null
    fi
}
